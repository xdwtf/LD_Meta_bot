...
def hrestart_mod(m):
    resmes = bot.send_message(m.chat.id, text="#Restart-Dynos - Rss Userbot", parse_mode=telegram.ParseMode.MARKDOWN)
    try:
        # Requests
...
def hrestartx_mod(m):
    resmes = bot.send_message(m.chat.id, text="#Restart-Dynosx - Leech Bot", parse_mode=telegram.ParseMode.MARKDOWN)
    try:
        # Requests
